...
from scipy import matrix, arcsin, sqrt
...
def matFAI(n1z, d1, k0):
    # Φ 行列の定義
    return matrix([[np.exp(1j * n1z * k0 * d1), 0], [0, np.exp(-1j * n1z * k0 * d1)]])
...
t1DegOut = np.linspace(t1start, t1end, t1points)    # 入射角 t1 の配列の生成
t1 = 0.25 * np.pi + (1 / n1) * arcsin((t1DegOut - 45) /
                                         180 * np.pi)  # 入射角をラジアンに直す
s1 = np.sin(t1)         # sin(t1)
c1 = np.cos(t1)         # cos(t1)
s2 = n1 / n2 * s1        # sin(t2)
c2 = np.sqrt(1 - s2**2, dtype=complex)   # cos(t2)
s3 = n1 / n3 * s1        # sin(t3)
c3 = np.sqrt(1 - s3**2, dtype=complex)   # cos(t3)
s4 = n1 / n4 * s1        # sin(t4)
c4 = np.sqrt(1 - s4**2, dtype=complex)   # cos(t4)
...
matT0 = np.zeros((t1points, 2, 2), dtype=complex)          # 伝搬行列 T0 の初期化
matT1 = np.zeros((t1points, 2, 2), dtype=complex)          # 伝搬行列 T1 の初期化
r0 = np.zeros((t1points), dtype=complex)                 # 反射係数(誘電体なし)の初期化
r1 = np.zeros((t1points), dtype=complex)                 # 反射係数(誘電体あり)の初期化
