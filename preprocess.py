...
class Preprocess():
    ...
    def _set_param(self, courantfac):
        self.dt = courantfac / (self.cc * math.sqrt(1.0 / self.dx /
                                                    self.dx + 1.0 / self.dy / self.dy + 1.0 / self.dz / self.dz))
    ...
    def _eps_sio2(self, wavelength):
        ...
        a4 = 4.85560050e-5
        a5 = 8.92406983e-7
        ee = a0 + a1 * wavelength**2 + a2 / wavelength**2 - a3 / \
            wavelength**4 + a4 / wavelength**6 - a5 / wavelength**8
        return ee
    ...
    def _sphere(self, obj):
        ...
        for iz in range(self.moz1, self.moz2):
            ...
            for iy in range(self.moy1, self.moy2):
                y = (iy - self.moy1 + 0.5) * \
                    self.dy - self.y0 - obj.position[1]
                for ix in range(self.mox1, self.mox2):
                    x = (ix - self.mox1 + 0.5) * self.dx - \
                        self.x0 - obj.position[0]
                    if x * x + y * y + z * z <= rr:
                        for jj in [0, 1]:
                            for ii in [0, 1]:
                                iix = ix
    ...
    def _slab(self, obj):
        ...
        materialid = self.name2mat[obj.material]
        izmin = math.ceil(
            (self.z0 + obj.position - obj.size / 2.0) / self.dz + self.moz1 - 0.5)
        izmax = math.floor(
            (self.z0 + obj.position + obj.size / 2.0) / self.dz + self.moz1 + 0.5)
    ...
    def _substrate(self, obj):
        ...
        materialid = self.name2mat[obj.material]
        izmin = math.ceil(
            (self.z0 + obj.position[2]) / self.dz + self.moz1 - 0.5)
        ...
        for fieldmon in fieldmons[2:]:
            if fieldmon.axis == 'x':
                ...
                if posi < 0 or posi > self.mzz:
                    print('Error: fieldmon location is out of range !')
                    sys.exit()
            prefix = './field/' + fieldmon.ehfield + '_' + \
                fieldmon.axis + '{0:0>3}'.format(posi0) + '_'
        ...
        for epsmon in epsmons:
            if epsmon.axis == 'x':
                ...
                if posi < 0 or posi > self.mzz:
                    print('Error: epsmon location is out of range !')
                    sys.exit()
            fname = './field/eps' + epsmon.pol + \
                '_' + epsmon.axis + str(posi0) + '.txt'
    ...
    def _cpmlparam(self, mpml, kappamax, amax, mpow):
        ...
        # x direction
        sigmamax = -(mpow + 1) * self.eps0 * self.cc * \
            math.log(1.0e-8) / (2.0 * mpml * self.dx)
        for ix in range(mpml + 1):
            ...
            self.cbxe[ix] = math.exp(-(alpha + sig / kappa)
                                     * (self.dt / self.eps0))
            self.ccxe[ix] = -(1.0 - self.cbxe[ix]) * \
                (sig / kappa) / (sig + alpha * kappa) / self.dx
        ...
        for ix in range(mpml):
            ...
            self.cbxh[ix] = math.exp(-(alpha + sig / kappa)
                                     * (self.dt / self.eps0))
            self.ccxh[ix] = -(1.0 - self.cbxh[ix]) * \
                (sig / kappa) / (sig + alpha * kappa) / self.dx
        ...
        # y direction
        sigmamax = -(mpow + 1) * self.eps0 * self.cc * \
            math.log(1.0e-8) / (2.0 * mpml * self.dy)
        for iy in range(mpml + 1):
            ...
            self.cbye[iy] = math.exp(-(alpha + sig / kappa)
                                     * (self.dt / self.eps0))
            self.ccye[iy] = -(1.0 - self.cbye[iy]) * \
                (sig / kappa) / (sig + alpha * kappa) / self.dy
        ...
        for iy in range(mpml):
            ...
            self.cbyh[iy] = math.exp(-(alpha + sig / kappa)
                                     * (self.dt / self.eps0))
            self.ccyh[iy] = -(1.0 - self.cbyh[iy]) * \
                (sig / kappa) / (sig + alpha * kappa) / self.dy
        ...
        # z direction
        sigmamax = -(mpow + 1) * self.eps0 * self.cc * \
            math.log(1.0e-8) / (2.0 * mpml * self.dz)
        for iz in range(mpml + 1):
            ...
            self.cbze[iz] = math.exp(-(alpha + sig / kappa)
                                     * (self.dt / self.eps0))
            self.ccze[iz] = -(1.0 - self.cbze[iz]) * \
                (sig / kappa) / (sig + alpha * kappa) / self.dz
        ...
        for iz in range(mpml):
            ...
            self.cbzh[iz] = math.exp(-(alpha + sig / kappa)
                                     * (self.dt / self.eps0))
            self.cczh[iz] = -(1.0 - self.cbzh[iz]) * \
                (sig / kappa) / (sig + alpha * kappa) / self.dz
    ...
    def _devparam(self):
        ...
        for imater in range(self.diemax):
            ...
            temp1 = self.eps0 * self.epsinf[imater] / \
                self.dt - self.sigma[imater] / 2.0
            temp2 = self.eps0 * self.epsinf[imater] / \
                self.dt + self.sigma[imater] / 2.0
        ...
        for imater in range(self.diemax, self.drumax):
            ...
            self.cj1[imater] = (1.0 - self.gamma[imater] * self.dt /
                                2.0) / (1.0 + self.gamma[imater] * self.dt / 2.0)
            self.cj3[imater] = (self.eps0 * self.omegap[imater] * self.omegap[imater]
                                * self.dt / 2.0) / (1.0 + self.gamma[imater] * self.dt / 2.0)

            temp1 = self.eps0 * self.epsinf[imater] / \
                self.dt - self.cj3[imater] / 2.0
            temp2 = self.eps0 * self.epsinf[imater] / \
                self.dt + self.cj3[imater] / 2.0

            self.ce1[imater] = temp1 / temp2
            self.ce2[imater] = 1.0 / temp2
            self.ce3[imater] = self.ce2[imater] * \
                (1.0 + self.cj1[imater]) / 2.0
