...
class plot2d (PlotBase):
    ...
    def pol_plot(self, tdeg, Rp, Tp, Rs, Ts, aspect="auto", pngname=None):
        self.new_fig(aspect=aspect)
        self.axs.plot(tdeg, Rp, label=r"$R_{12}^{\rm{p}}$",
                      linewidth=3.0, color='black', linestyle='dashed')
        self.axs.plot(
            tdeg, Tp, label=r"$T_{12}^{\rm{p}}$", linewidth=3.0, color='black')
        self.axs.plot(tdeg, Rs, label=r"$R_{12}^{\rm{s}}$",
                      linewidth=3.0, color='gray', linestyle='dashed')
        self.axs.plot(
            tdeg, Ts, label=r"$T_{12}^{\rm{s}}$", linewidth=3.0, color='gray')
    ...
    def pol_plot_ref(self, tdeg, Rp, Rs, aspect="auto", pngname=None):
        self.new_fig(aspect=aspect)
        self.axs.plot(
            tdeg, Rp, label=r"$R_{12}^{\rm{p}}$", linewidth=3.0, color='black')
        self.axs.plot(
            tdeg, Rs, label=r"$R_{12}^{\rm{s}}$", linewidth=3.0, color='gray')
        self.axs.set_xlabel("Inject (deg.)", fontsize=20)
        self.axs.set_ylabel("Reflect", fontsize=20)
